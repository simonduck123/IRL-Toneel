...
namespace Katpatat.Networking.Utils
{
    public static class NetworkMessageUtil {
        ...
        // ----- SWIMMING GAME ----- //
        public static event Action<string, string, float, float, string> OnSwimLocation;    // idModule, idPlayer, x, y, nickname
        public static event Action<string, string, string> OnPlayerRemove;              // idModule, idPlayer, nickname
        public static event Action<string, string, string, string> OnSwimPlayerAction;      // idModule, idPlayer, actionName, nickname
        ...
        public static event Action<PlayerPosition[]> OnRiderPositions;   // id, PlayerPosition[]
        public static event Action<string> OnRiderExplosion;   // id
        public static event Action<string,string> OnRiderJoined;   // id, nickname
        ...
        public static void HandleMessage(NormalMessage message)
        {
            switch (message.header) {
                ...
                // ----- MOTOR GAME ----- //
                case "rider-all-positions":
                    OnRiderPositions?.DynamicInvoke(ConvertArguments(OnRiderPositions, message.args));
                    break;
